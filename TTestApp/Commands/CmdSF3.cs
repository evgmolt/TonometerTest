...
{
    //Управление макетом SF3 осуществляется передачей ему команд от компьютера.
    //В ответ на каждую команду макет SF3 посылает пакет длиной 3 байта, содержащий значение давления.
    //Параметры интерфейса: скорость 115200, 8 бит, контроля четности нет, 1 стоп бит.
    internal enum CmdSF3 : byte
    {
